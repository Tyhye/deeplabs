...
'''
 @Author: tyhye.wang 
 @Date: 2018-07-24 00:08:49 
 @Last Modified by:   tyhye.wang 
 @Last Modified time: 2018-07-24 00:08:49 
'''
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None):
        ...
        self.conv2 = conv3x3(planes, planes, stride=1, padding=dilation, dilation=dilation)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, mg=[1, 2, 4], output_stride=8):
        ...
        self.bn1 = nn.BatchNorm2d(64) #, eps=1e-5, momentum=0.999, affine=True)
        self.relu = nn.ReLU(inplace=True)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, mg=None):
        ...
        dilation_ = dilation
        if mg is not None:
            dilation_ = dilation * mg[0]
        layers.append(block(self.inplanes, planes, stride,
                            dilation_, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            if mg is not None:
                dilation_ = dilation * mg[i]
